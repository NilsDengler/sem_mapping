  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        if(class_id in [381]):
            class_id = 135
            class_name = 'Computer monitor'
        elif(class_id in [121,544,378]):
            class_id = 281
            class_name = 'Table'
        elif(class_id in [334,515]):
            class_id = 233
            class_name = 'Houseplant'
        elif(class_id in [171]):
            class_id = 179
            class_name = 'Coffee cup'
        elif(class_id in [170]):
            class_name = 'Cola can'
        if(class_id not in [241, 462, 130, 466,442] and class_name!='N/A'):
          data.append([class_id,class_score,box,class_name])
